...
public class BooleanExpressions extends CommonPolicy {
    ...
    public boolean isMatch(Node node, VariableTypeMap typeMap) {
        ...
        if (!parsedExpr.isLHSVariable() && !parsedExpr.getType().isUndefined() &&
                parsedExpr.getType().isBooleanType()){
            ...
            if (parentNode != null){
                ...
                return !parsedParentExpr.getType().isBooleanType();
            } else {
                return true;
            }
        } else {
    ...
    public void filterOptions(List<CodeBERTOption> options, Node node, VariableTypeMap typeMap) {
        ...
        for (CodeBERTOption option : options){
            ...
            if (!parsedExpression.getType().isUndefined() && !parsedExpression.getType().isBooleanType()){
                toRemove.add(option);
            }
        }
