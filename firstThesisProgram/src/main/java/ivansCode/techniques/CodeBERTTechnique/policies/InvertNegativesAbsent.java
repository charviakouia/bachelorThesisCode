...
public class InvertNegativesAbsent extends CommonPolicy {
    ...
    public boolean isMatch(Node node, VariableTypeMap typeMap) {
        ...
        boolean isNonNegatedRHSVariable =
                ...
                !(NodeUtils.followsHierarchy(node, NameExpr.class, UnaryExpr.class)
                        && NodeUtils.unaryOperatorIn((UnaryExpr) NodeUtils.up(node, 2), forbiddenOperators));
        return isNonNegatedRHSVariable && parsedExpression.getType().isNumericalType();
    }
